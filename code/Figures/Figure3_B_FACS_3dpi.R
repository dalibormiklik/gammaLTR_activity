...
#Due to the large size of the original file exported from the FlowJo, modified 'Cell_Export_2.txt' - originally the 'data.to.plot' data.frame file is loaded instead
#File contains max. 10k cell per sample randomly selected by the 'sample()' function (see additional code below)
export.dir <- data.path
export.file <- "Cell_Export_2.txt"
...
#This code is included when the original 'Cell_Export.txt' table is loaded
...
#                               }))
#
##This 'data.to.plot' data.frame was saved as:
#write.table(data.to.plot, paste0(export.dir, Cell_Export_2.txt),
#            quote = FALSE, sep = "\t", col.names = TRUE)

sel.export.df <- export.df
data.to.plot <- export.df
